...
from nupic.torch.modules import SparseWeights, KWinners, rezero_weights
...
class Net(nn.Module):
    def __init__(
        ...
        self.kw_percent_on = 0.25
        self.weight_sparsity = 0.1
        ...
        for i, next_shape in enumerate(append_hidden_shapes):
            ...
            fc = SparseWeights(module=fc, sparsity=self.weight_sparsity, allow_extremes=True)

            # append_hidden_init_func(fc)
            if i == 0:
                self._init_sparse_weights(fc, 0.0)
            else:
                self._init_sparse_weights(fc, 1 - self.kw_percent_on if self.kw_percent_on else 0.0)
            ...
            self.activation_funcs.append(KWinners(n=next_shape,
                                      percent_on=self.kw_percent_on,
                                      k_inference_factor=1.0,
                                      boost_strength=0.0,
                                      boost_strength_factor=0.0))

        self.last = nn.Linear(append_input_shape, output_shape)
        self.last = SparseWeights(module=self.last, sparsity=self.weight_sparsity, allow_extremes=True)
        self._init_sparse_weights(self.last, 1 - self.kw_percent_on if self.kw_percent_on else 0.0)

        print('Using sparse network!')
    ...
    def forward(self, x):
        ...
        for append_fc, activation_func in zip(self.append_fcs, self.activation_funcs):
            out = append_fc(out)
