...
for eachcolor, eachlinestyle, exp_name, exp_tag in zip(colors, linestyles_choose, args.id, args.tags):
    ...
    ax1.plot(final_step, all_mean, label=exp_tag, color=eachcolor, linestyle=eachlinestyle, linewidth=3,
             marker='d', markeredgewidth=4, markersize=4, markevery=500)
    # ax1.plot(final_step, all_mean, label=exp_tag, color=eachcolor, linestyle=eachlinestyle, linewidth=3)
...
ax1.set_ylabel('Success Rates', fontsize=30)
...
# leg = ax1.legend(bbox_to_anchor=(1.05, 1.0), loc='upper left')
# plt.tight_layout()
...
plt.savefig(os.path.join(args.output_dir, '{}_{}{}.png'.format(env_name, args.entry, args.add_tag)), bbox_inches='tight')
plt.show()
plt.close()
