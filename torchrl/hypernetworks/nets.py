...
import torchrl.hypernetworks.modules as modules
...
class FlattenNet(Net):
    def __init__(self, output_shape, base_type, **kwargs):
        ...
        # self.model = modules.HyperMTANProSparse(hyper_input_dim=self.task_num, mlp_input_dim=9 + 4, mlp_output_dim=1, is_share=False)
        # self.model = modules.HyperMLP(hyper_input_dim=self.task_num, mlp_input_dim=9 + 4, mlp_output_dim=1)

    def forward(self, input):
        out = self.model(input[0][..., -self.task_num:], torch.cat([input[0][..., :9], input[1]], dim=-1))
