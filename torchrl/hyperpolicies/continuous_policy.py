...
class GuassianContPolicy(networks.Net):
    def __init__(self, output_shape, base_type, **kwargs):
        ...
        # self.model = modules.HyperMTANProSparse(hyper_input_dim=self.task_num, is_share=False)
        # self.model = modules.HyperMLP(hyper_input_dim=self.task_num)


    def forward(self, x):
        x = self.model(x[..., -self.task_num:], x[..., :9])
